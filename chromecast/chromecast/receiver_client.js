...
const context = window.castContext;
const CHANNEL = window.CAST_CHANNEL;
...
context.addCustomMessageListener(CHANNEL, function (customEvent) {
  ...
  const jsonData =
    typeof customEvent.data === "string"
...
// 创建PeerConnection
function setupPeerConnection(options = {}) {
  ...
  if (options.createDataChannel) {
    bindDataChannel(pc.createDataChannel("channel"));
  }
